    ...
    <div className={`flex flex-col items-center justify-end h-full transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`}>
      <div
        className={`
          ...
          ${isExpanded ? "h-120 justify-start" : "h-64 justify-start"}
        `}
      >
        ...
        <div className="px-8 w-full flex flex-col items-center relative z-10 flex-1 min-h-0">
          ...
          <div className={`w-full flex justify-center shrink-0 border-b transition-all duration-500 ${isExpanded ? "border-slate-800/50 pb-6 mb-6" : "border-transparent pb-0 mb-4"}`}>
          ...
          {!isExpanded ? (
            ...
            <div className={`space-y-6 text-xl text-center text-slate-300 leading-relaxed max-w-xl pb-6 mt-4 transition-opacity duration-700 w-full flex-1 ${showNarrative ? "opacity-100" : "opacity-0"} ${isScrollable ? "overflow-y-auto scrollbar-thin scrollbar-thumb-emerald-900 scrollbar-track-transparent" : "overflow-hidden"}`}>
          ...
          {isExpanded ? (
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                aria-label="Close"
              >
                <span className="text-sm tracking-widest uppercase border-b border-transparent hover:border-white/50 pb-1">Close</span>
              </button>
            </div>
          )}
        </div>
